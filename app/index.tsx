...
/**
 * Komponen ImageGrid
 * Menampilkan grid gambar 3x3 yang interaktif dengan fitur:
 * - Setiap gambar memiliki versi utama dan alternatif
 * - Klik pertama: ganti gambar ke versi alternatif
 * - Klik kedua: gambar membesar 1.2x
 * - Klik ketiga: gambar membesar maksimal 2x dan tidak bisa diklik lagi
 * 
 * Grid responsive di berbagai perangkat (mobile & web)
 */
...
// Data gambar: 9 pasang gambar utama dan alternatif
const images = [
  {
...
export default function ImageGrid() {
  // Mengambil ukuran layar secara dinamis agar responsif
  const { width } = useWindowDimensions();
  const cellMargin = 8;
  const cellSize = (width - cellMargin * 4) / 3; // margin 4 kali (2 sisi + antar cell)

  // State untuk tiap gambar: jumlah klik, gambar alternatif aktif, dan skala
    ...
    setStates((prev) =>
      prev.map((item, i) => {
        ...
        return {
          ...
          isAlt: true, // langsung aktifkan gambar alternatif di klik pertama
          scale:
            nextClick === 1
              ? 1 // klik pertama ganti gambar saja
              : nextClick === 2
              ? 1.2 // klik kedua mulai scale 1.2x
              : 2,   // klik ketiga scale maksimal 2x
        };
      })
    );
  };
        ...
        return (
          <TouchableOpacity
            ...
            style={[
              ...
              {
                width: cellSize,
                height: cellSize / (3 / 4), // jaga rasio 3:4
              },
            ]}
            activeOpacity={0.8}
            disabled={current.clickCount >= 3} // klik berhenti setelah 3 kali
          >
            <Image
              source={{ uri: source }}
              style={[
